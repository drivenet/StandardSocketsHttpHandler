        ...
        internal async Task<(HttpConnection, HttpResponseMessage)> CreateConnectionAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                switch (_kind)
                {
                    ...
                    case HttpConnectionKind.Https:
                    case HttpConnectionKind.ProxyConnect:
                        (socket, stream) = await ConnectHelper.ConnectAsync(_host, _port, request, _poolManager.Settings._connectCallback, cancellationToken).ConfigureAwait(false);
                        break;

                    case HttpConnectionKind.Proxy:
                        (socket, stream) = await ConnectHelper.ConnectAsync(_proxyUri.IdnHost, _proxyUri.Port, request, _poolManager.Settings._connectCallback, cancellationToken).ConfigureAwait(false);
                        break;
